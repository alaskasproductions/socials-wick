...
import { startStripePaymentAction } from "@/lib/actions/customer";

const PRESETS = [10, 25, 50, 100];

export default function StripePaymentForm() {
  const [state, formAction, pending] = useActionState(startStripePaymentAction, undefined);
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      >
        {pending ? "Redirecting to checkout…" : "Pay with Card — via Stripe"}
      </button>
      <p className="text-xs text-slate-500">
        You'll be redirected to Stripe's secure checkout. Your balance is credited automatically
